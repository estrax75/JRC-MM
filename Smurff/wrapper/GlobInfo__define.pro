FUNCTION GlobInfo::getGlob, globCode

  glob=obj_new('GlobPar')
  idx=self->getIndexByCode(globCode)
  fNames=self->getFullFileNames()
  for i=0, n_elements(fNames)-1 do doLog, 'fNames list...', fNames[i]
  doLog, 'fNames-->', fNames[idx], '<--', level=4 
  res = glob->fillDataFromXMLFile(fNames[idx])
  return, glob  
...
FUNCTION GlobInfo::getFullElement, code, INDEX=INDEX
...
FUNCTION GlobInfo::getIndexByCode, code
...
FUNCTION GlobInfo::buildFullFieldList, simpleList
...
FUNCTION  GlobInfo::getVersion

  return, '1.0'
  
END

FUNCTION GlobInfo::buildRecordFromElement, index, structElement
...
FUNCTION GlobInfo::getDisplayNames
...
FUNCTION GlobInfo::getFullFileNames
...
FUNCTION GlobInfo::getListStructDef
...
FUNCTION GlobInfo::buildListStruct, fieldsInfo, NOFILL=NOFILL
...
FUNCTION GlobInfo::listFieldsExpectedNumber

  return, n_tags(self->buildListStruct(/NOFILL))
  
END

FUNCTION GlobInfo::init, application, filename, mode=mode
...
PRO GlobInfo::cleanUp

  self -> ConfigurableData::cleanUp
  
END

PRO GlobInfo__Define
