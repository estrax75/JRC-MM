...
function read_AVHRR_multi, folder, fileName, FOUND=FOUND, $
...
;  declareSingleTons
  ...
  if keyword_set(PIXELS_PROCESS) then begin
    if keyword_set(PP_CSV) then allData=readSPP_AVHRR_FromCSV(folder, fileName, FOUND=FOUND, ORIGDIMS=ORIGDIMS)
    if keyword_set(PP_NC) then allData=readSPP_AVHRR_FromNC(folder, fileName, FOUND=FOUND, ORIGDIMS=ORIGDIMS)
  endif else begin
    allData=readMatrix_AVHRR_multiFromNC(folder, fileName, bandName, FOUND=FOUND, FULL=FULL, USENAN=USENAN, APPLY_CONVERSION=APPLY_CONVERSION)
  endelse
