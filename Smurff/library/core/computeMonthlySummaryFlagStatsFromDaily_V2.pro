...
function computeMonthlySummaryFlagStatsFromDaily_V2, operator, roiNo, singleStatToApply, periodtype, $
  ...
  reportMonthlyTimeSeriesDataCheck7=fltarr(roiNo, 1,  monthlyBinsNumber, binNum, flagInfoNum)
  ...
  fullFileNameHistoCheck7=strarr(flagStatNum, binNum, flagInfoNum)
  ...
  histoMainTitlesCheck7=strarr(flagStatNum, binNum, flagInfoNum)
  ...
  for statIndex=0, flagStatNum-1 do Begin
    for binIndex=0, binNum-1 do begin
      ...
      for jj=0, flagInfoNum-1 do begin
        ...
        histoMainTitlesCheck7[statIndex, binIndex, jj]='Flagged Cases ('+trueFlagInNames[jj]+': F!D'+trueFlagInCodes[jj]+'!N!X '+neqSymbol+strcompress(binThresholds[binIndex], /REMOVE)+')'
        fullFileNameHistoCheck7[statIndex, binIndex, jj]=operator->buildOperatorResultFileName(periodType, prefix7+fileDesc, monthList, yearList, singleStatToApply, '', outputDir+path_sep()+histoFolder, INTERVAL=INTERVAL, /FULLPATH)
      endfor
      ;histoMainTitlesCheck7[statIndex, binIndex, jj]='Flagged Cases ('+mul
;      mul=''
;      for jj=0, flagInfoNum-1 do begin
;        fileDesc='_'+trueFlagInNames[jj]
;        if jj ne 3 then begin
;          mul=mul+'F!D'+trueFlagInCodes[jj]+'!N!X '+neqSymbol+strcompress(binThresholds[binIndex], /REMOVE)
;          if jj lt 2 then mul=mul+'||' else mul=mul+')'
;        endif
;        histoMainTitlesCheck7[statIndex, binIndex, jj]='Flagged Cases ('+trueFlagInNames[jj]+': F!D'+trueFlagInCodes[jj]+'!N!X '+neqSymbol+strcompress(binThresholds[binIndex], /REMOVE)+')'
;        fullFileNameHistoCheck7[statIndex, binIndex, jj]=operator->buildOperatorResultFileName(periodType, prefix7+fileDesc, monthList, yearList, singleStatToApply, '', outputDir+path_sep()+histoFolder, INTERVAL=INTERVAL, /FULLPATH)
;      endfor
;      histoMainTitlesCheck7[statIndex, binIndex, jj]='Flagged Cases ('+mul
  ...
  for k=0, roiNo-1 do begin
    ...
    for m=0, yearNo-1 do begin
      ...
      for n=0, monthNo-1 do begin
        ...
        ; Third index is total count of exclusive flags
        zeroFlagsCondition=' and ('+flagDataPrefix+'3'+' eq 0) '
        ...
        for statIndex=0, sigmaStatNum-1 do begin
          ...
          ; 2) Zero "true" flags condition
          zeroFlagsCheck=checkStringPart1+checkStringPart3+zeroFlagsCondition+checkStringPart4+'ZERO_FLAGS)'
          res=execute(zeroFlagsCheck)
          ;print, countDaysPOS_ZERO_FLAGS

          if binThresholds[binIndex] eq 0 then symbol='= ' else symbol=leqSymbol;cgSymbol('leq')
          histoMainTitlesCheck2[*,*]=yGraphicDisplayName+': '+' F!DE!N!X '+symbol+strcompress(binThresholds[binIndex], /REMOVE)
          ...
          reportMonthlyTimeSeriesDataCheck2[k, parDataIndex, timeIndex,0]=countDaysZERO_FLAGS

          multipleCondition='('
          for jj=0, flagInfoNum-2 do begin
            indexAsString=strcompress(jj, /REMOVE)
            flagTSDDataString=flagDataPrefix+indexAsString
            flagTSDDataString='('+flagTSDDataString+' ne 0)'
            multipleCondition= multipleCondition+flagTSDDataString
            if jj ne flagInfoNum-2 then multipleCondition=multipleCondition+' or '
            stringToExec=checkStringPart1+flagTSDDataString+checkStringPart4+'SINGLE_FLAG)'
            res=execute(stringToExec)
            reportMonthlyTimeSeriesDataCheck7[k, parDataIndex, timeIndex,0,jj]=countDaysSINGLE_FLAG
          endfor
          multipleCondition=multipleCondition+')'
          stringToExec=checkStringPart1+multipleCondition+checkStringPart4+'MULTIPLE_FLAG)'
          res=execute(stringToExec)
          reportMonthlyTimeSeriesDataCheck7[k, parDataIndex, timeIndex,0,flagInfoNum-1]=countDaysMULTIPLE_FLAG
          ...
          for cvCheckIndex=0, cvNum-1 do begin
            ...
            histoMainTitlesCheck4[statIndex, cvCheckIndex, 0]='Homogeneus Cases ('+yGraphicDisplayName+':'+' F!DE!N!X '+symbol+strcompress(binThresholds[binIndex], /REMOVE)+' '+andSymbol+' '+'C!Dv!N!X '+'< '+string(format='(f3.1)', cvThresholds[cvCheckIndex])+')'
            histoMainTitlesCheck5[statIndex, cvCheckIndex, 0]='Homogeneus Cases ('+yGraphicDisplayName+':'+' F!DE!N!X '+symbol+strcompress(binThresholds[binIndex], /REMOVE)+' '+andSymbol+' '+'C!Dv!N!X '+'< '+string(format='(f3.1)', cvThresholds[cvCheckIndex])+' '+andSymbol+' '+'QC!Ds!N!X '+')'
            ...
            for kk=0, thrInParNum-1 do begin
              ...
              title1=title1+' F!DE!N!X '+symbol+strcompress(binThresholds[binIndex], /REMOVE)+' '+andSymbol
              title1=title1+' C!Dv!N!X '+'< '+string(format='(f3.1)', cvThresholds[cvCheckIndex])+' '+andSymbol
              title1=title1+' '+thrInParNames[kk]+' '+leqSymbol+string(format='(f3.1)', meanThresholds[kk])+')'
              ...
              ; 6a/b/c) Rrs cv AND zero flags AND multiple pars thresholds
            ...
            ; 5) Rrs cv AND zero flags AND multiple pars thresholds
  ...
  monthlyData={ reportMonthlyTimeSeriesDataCheck2:reportMonthlyTimeSeriesDataCheck2, $
