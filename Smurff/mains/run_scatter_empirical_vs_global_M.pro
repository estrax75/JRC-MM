PRO run_scatter_empirical_vs_global_M, request, NODISPLAY=NODISPLAY
    ...
    for j=0, n_elements(monthList)-1 do begin
      ...
      for k=0, n_elements(roiCodeList)-1 do begin
        ...
        if ~keyword_set(NOTFOUND) then begin
          ...
          title=monthList[j]+' - '+year+' - '+roiInfo.displayName+' - '+'Empirical Vs ' + 'Global';physical->getDisplayName()
