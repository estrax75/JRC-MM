PRO computeSingleSatelliteExtraction_v7
  ...
  sensorList=['JRCSEAWIFS', 'SEAWIFS']
  ;siteList=['LSea', 'NPO', 'CSea', 'EIO', 'ASea', 'MSea']
  ...
  for i=0, n_elements(sensorList)-1 do begin
    ...
    for j=0, n_elements(siteList)-1 do begin
      thisSite=siteList[j]
      fileToRead1=statDir+'satInfo'+'_'+thisSensor+'_'+thisSite+'.sav'
      fileToRead2=statDir+'advSatInfo'+'_'+thisSensor+'_'+thisSite+'.sav'
      summaryStats[l].site=thisSite
      summaryStats[l].sensor=thisSensor
      delIDLVar, satInfo
      delIDLVar, destSat
      fileName1=file_Search(fileToread1, FOLD_CASE=1)
      fileName2=file_Search(fileToread2, FOLD_CASE=1)
      notfound=1
      if fileName1 ne '' then begin
        restore, fileName1
        notfound=0
      endif else begin
        if fileName2 ne '' then begin
          restore, fileName2
          satInfo=destSat
          notfound=0
        endif
...
;        store-->mwind           6        1999
        ...
        if ~keyword_set(NOFLAG) then for m=0, n_elements(flagToBit)-1 do destSat[index].flagsTotal[m]=total(*(flagToBit[m]))
         ...
         matchUPStruct=computeMatchUp_v7(flagData, flagToBit, $
      ...
      print, '22) -->', total(destSat[cc].matchUpResults[22])
      print, '23) -->', total(destSat[cc].matchUpResults[23])
      print, '24) -->', total(destSat[cc].matchUpResults[24])
      ; 
