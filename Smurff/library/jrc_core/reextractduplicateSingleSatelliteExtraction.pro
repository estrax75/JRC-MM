PRO reextractduplicateSingleSatelliteExtraction
  ...
  for i=0, n_elements(sensorList)-1 do begin
    ...
    for j=0, n_elements(siteList)-1 do begin
      ...
      sourceFiles=destSat[*].sourceFile
      destSat[*].assignedExtractionFile=satInfo[*].assignedExtractionFile
      potentialDuplicateFiles=satInfo[*].assignedExtractionFile
      sorting=sort(potentialDuplicate)
      potentialDuplicateFiles=potentialDuplicateFiles[sorting]
      sourceFiles=sourceFiles[sorting]
      for l=0, n_elements(potentialDuplicateFiles) do begin
