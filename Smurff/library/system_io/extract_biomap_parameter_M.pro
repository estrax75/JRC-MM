FUNCTION extract_biomap_parameter_M, periodType, $
  ...
  bioMapStatOperator=obj_new('BiomapStatisticsOperator', mainApp, tempDir, periodType, statBandCode=displayName, bandToExportList=statToCompute)
  ; check main code and archive code also (just to be sure)
  roiInfo=mainApp->getROIInfoByCode(roiCode)
  fileName1=bioMapStatOperator->buildOperatorResultFileName(periodType, statToCompute, month, year, sensorCode, roi)
  fileName2=bioMapStatOperator->buildOperatorResultFileName(periodType, statToCompute, month, year, sensorCode, roiInfo.archiveCode)
  fullFileNames=strarr(2)
  fullFileNames[0]=biomapOutputDir+path_sep()+fileName1
  fullFileNames[1]=biomapOutputDir+path_sep()+fileName2
  
  
  for i=0, n_elements(fullFileNames)-1 do begin
    fullFileName=fullfileNames[i]
    data=biomapStatOperator->readNcdfVar(fullFileName, parCode, FOUND=FOUND1, /REVERSE, TRANSPOSE=TRANSPOSE)
  ...
  if keyword_set(NOTFOUND1) and n_elements(refROI) eq 1 then begin
    ...
    fileName1=biomapStatOperator->buildOperatorResultFileName(periodType, statToCompute, month, year, sensorCode, refRoi)
    fileName2=biomapStatOperator->buildOperatorResultFileName(periodType, statToCompute, month, year, sensorCode, refRoiArchiveCode)
    refRoiFullFileNames=[biomapOutputDir+path_sep()+fileName1, biomapOutputDir+path_sep()+fileName2]
  ...
  if ~keyword_set(NOTFOUND2) then begin
    bandInfos=biomapStatOperator->prepareBands(refRoiFullFileNames, parCode, NOTFOUND=NOTFOUNDCROP)
    if keyword_set(NOTFOUNDCROP) then return, -1
    dataInfo=bioMapStatOperator->cropping(bandInfos, parCode, $
