FUNCTION do_single_e_regional_M, periodType, $
  ...
  finalBaseFileName=call_function('buildERegionalFileName'+"_"+periodType, $
  ...
  if ((keyword_set(overwriteFlag) and count gt 0) or (count eq 0)) then begin
    ...
    eRegFile=fs->removeFileExtension(roiSeaWiFSFile)
    
    eRegOp=obj_new('ERegionalOperator', mainApp, tempDir, /ENVITYPE, fileName=eRegFile, bandToExportList=bandToExportList)
    productCodes = eRegOp->getBandToExportList()
    
    for i=0, n_elements(productCodes)-1 do begin
      NO_DATA=0
      finalBioMapFile=eRegOp->doComputation(roi, productCodes[i], sensorCode, applyfoq=applyfoq, ignoreValue=ignoreValue, textInputFile=textInputFile, pixelMask=pixelMask, NO_DATA=NO_DATA)
      if keyword_set(NO_DATA) then break
    endfor
    NOTFOUND=NO_DATA
    if ~keyword_set(NO_DATA) then begin
      eRegOp->writeAsNCDF, month, year, sensorCode, roi, archiveDir=outputDir, overwriteFlag=overwriteFlag, fileToOverwriteList=fileToOverwriteList, applyFormula=applyFormula
