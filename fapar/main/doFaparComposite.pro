FUNCTION doFaparComposite, sensor, resolution, missionName, level, confDir, sourceDir, tempDir, outputDir, year, month, $
  ...
  for type=0, n_elements(first)-1 do begin
    ...
    while checkFirst do begin
      ...
      if sensor eq 'AVH09C1' then sourceFileName=buildAVHRRFAPARFileName_D(sensor, resolution, year, month, firstDay, missionName, missionCode, mainVarName, level, VERSION='01')
      if sensor eq 'MODIS' then sourceFileName=buildMODISFAPARFileName_D(sensor, resolution, year, month, firstDay, missionName, missionCode, mainVarName, level)
      ...
      print, 'file-->', ff1
      print, '*********'
      faparData=read_AVHRR_FAPAR(dir_in, ncfilename, FOUND=FOUND)
      faparData.valid=1
    ...
    for d=firstDay+1, last[type] do begin
      ...
      if sensor eq 'AVH09C1' then nsourceFileName=buildAVHRRFAPARFileName_D(sensor, resolution, year, month, d, missionName, missionCode, mainVarName, 'L2', VERSION='01')
      if sensor eq 'MODIS' then nsourceFileName=buildMODISFAPARFileName_D(sensor, resolution, year, month, d, missionName, missionCode, mainVarName, 'L2')

      ncfilename=fsObj->addFileExtension(nsourceFileName, 'NC'); nc
    ...
    if TA_TYPE eq 'MEAN' then begin
      ...
      if sensor eq 'AVH09C1' then destTCFile=buildAVHRRFAPARFileName_Mean(sensor, resolution, year, month, first[type], missionName, missionCode, mainVarName, startDay=first[type], endDay=last[type], level, VERSION='01')
      if sensor eq 'MODIS' then destTCFile=buildMODISFAPARFileName_Mean(sensor, resolution, year, month, first[type], missionName, missionCode, mainVarName, startDay=first[type], endDay=last[type], level, VERSION='01')
    ...
    if TA_TYPE eq 'TC' then begin
      ...
      endTime=systime(1)-starttime
      print, 'computed in about:', strcompress(endTime), 'seconds'
      if sensor eq 'AVH09C1' then destTCFile=buildAVHRRFAPARFileName_TC(sensor, resolution, year, month, first[type], missionName, missionCode, mainVarName, level, startDay=first[type], endDay=last[type])
      if sensor eq 'MODIS' then destTCFile=buildMODISFAPARFileName_TC(sensor, resolution, year, month, first[type], missionName, missionCode, mainVarName, level, startDay=first[type], endDay=last[type])
      destncfilename=fsObj->addFileExtension(destTCFile, 'NC'); nc
      desthdffilename=fsObj->addFileExtension(destTCFile, 'HDF'); hdf
      print, '*********'
