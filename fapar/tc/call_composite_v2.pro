; =======================================================================
;
Pro make_tc_distance_eu_vegetation_m, daysNumber, data_in, idx_doIt, day, meandat, std_mean, nfield, index_2
  ;
  ;
  ...
  ;
  ;
  FindEuclideanMatricDistance, daysNumber, data_in, idx_doIt, distance, meandat, std_mean, nfield
  ;
  ;
  ;
  ...
  if nfield eq 2 then idx_remake=where(data_in.flag eq 4.0 and index_2 ge 3)
  ;
  if idx_remake(0) ge 0 then begin
    print, 'Remake after Outliers out'
    FindEuclideanMatricDistance, daysNumber, data_in, idx_remake, distance_2, meandat2, std_mean2, nfield
...
;
;
PRO call_composite, daysNumber, data_day, data_tc
  ;
  ;
  ;
  ...
  ;
  ;
  make_tc_distance_eu_vegetation_m, daysNumber, data_day, idx_third, day, meandat, std_mean, 3, index_2
  ;
  ;stop
  ;
  ;
  ...
  ;
  make_tc_distance_eu_vegetation_m, daysNumber, data_day, idx_thirds, days, meandats, std_means, 2, index_2s
  ;
  ;
  for t =0 , tt(1) -1 do begin
    ;
    idx_t=where(days eq float(t))
    ;
    data_tc.red(idx_t)= data_day(t).red(idx_t)
    data_tc.nir(idx_t)= data_day(t).nir(idx_t)
    data_tc.fapar(idx_t)= data_day(t).fapar(idx_t)
    data_tc.flag(idx_t)= data_day(t).flag(idx_t)
    data_tc.day(idx_t) = days(idx_t)
    ;
    data_tc.sigma_red(idx_t)= data_day(t).sigma_red(idx_t)
    data_tc.sigma_nir(idx_t)= data_day(t).sigma_nir(idx_t)
    data_tc.sigma(idx_t)= data_day(t).sigma(idx_t)
