FUNCTION do_single_e_regional_D_to_M, periodType, $
  ...
  for day=firstDay, lastDay do begin
    ...
    regionalOp=obj_new('ERegionalOperator', mainApp, tempDir, periodType, /ENVITYPE, fileName=dataFile, bandToExportList=bandToExportList, /COPY)
    resFileList=regionalOp->getFileToOverwriteList(day, year, outputDir, sensorCode, roi, overwriteFlag=overwriteFlag, parInfo=parInfo, NONE=NONE, fileToPreserveNo=fileToPreserveNo, fileToPreserveIdx=fileToPreserveIdx)
    productCodes = regionalOp->getBandToExportList()
    if ~keyword_set(NONE) then begin
      for j=0, n_elements(productCodes)-1 do begin
        NO_DATA=0
        finalRegionalFile=regionalOp->doComputation(roi, productCodes[j], sensorCode, applyfoq=applyfoq, ignoreValue=ignoreValue, pixelMask=pixelMask, NO_DATA=NO_DATA)
    ...
    if ~keyword_set(NO_DATA) then begin
      if intermediateWrite then begin
        ;HISTOSTRUCT={ folder: mainApp->getKeyValue('TEMP_DIR'), $
        HISTOSTRUCT={ folder: getIntermediateDataDir(outputDir), $
          ...
          cutvalue: -1., $
          binsize: -1. }
        regionalOp->writeAsNCDF, day, year, sensorCode, roi, archiveDir=getIntermediateDataDir(outputDir), overwriteFlag=overwriteFlag, fileToOverwriteList=resFileList, /JULDAY, applyFormula=applyFormula, DOHISTO=mainApp->isTestMode(), HISTOSTRUCT=HISTOSTRUCT
      endif
      for k=0, bandToComputeNo-1 do begin
        bandValues=regionalOp->getBand(bandToExportList[k])
        ;bandValues=mainApp->applyRangeConditions(bandToExportList[k], bandValues, bandVarName, ignoreValue=ignoreValue)
    ...
    objToDestroy=[objToDestroy, regionalOp]
