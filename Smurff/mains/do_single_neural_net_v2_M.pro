FUNCTION do_single_neural_net_v2_M, periodType, $
  ...
  neuralNetConfFile=mainApp->getKeyValue('NEURAL_NET_V2_CONF_FILENAME')
  neuralNetDataFile=mainApp->getKeyValue('NEURAL_NET_V2_DATA_FILENAME')
  neuralNetFQFile=mainApp->getKeyValue('NEURAL_NET_V2_FQ_FILENAME')
  ...
  if ((keyword_set(overwriteFlag) and count gt 0) or (count eq 0)) then begin
    ...
    neuralNetOp=obj_new('NeuralNetOperatorV2', mainApp, tempDir, confFile=neuralNetConfFile, /ENVITYPE, fileName=neuralNetFile, bandToExportList=bandToExportList)
