PRO run_split_plot_rrs_homogeneus_final_check_report_time_series_abs_mr_D, request, NODISPLAY=NODISPLAY
  ...
  checkThr=fix(sCode[1])
  sensorCode=sCode[0]
  
  numInPar=numTotInPar-1
  cvParNum=numInPar-checkThr
  thrParNum=checkThr
...
meanTimeSeriesData=fltarr(roiNo, numInPar,  totDaysNumber, /NOZERO)
          ...
          if outPercentage gt compareValidityThreshold then begin
            ...
            for l=0, numInPar-1 do begin
              if ptr_valid(bands[l]) then begin
                ...
                if total(lastdim) ne total(thisDim) then continue
                lastDim=thisDim
                band=*bands[l]
                ...
                statRes=doStat(band, resSingleStatToApply)
                extraStatRes=doStat(band, 'stddev')
                extraStatRes2=doStat(band, 'coeffvar')
                meanTimeSeriesData[k, l, totalDayCount]=statRes.statValue
                ...
                cvTimeSeriesData[k, l, totalDayCount]=extraStatRes2.statValue
            ...
            test=fix(total(finite(meanTimeSeriesData[k, *, totalDayCount])))
            if test ne n_elements(bands) then begin
              ...
              if climFlag ne 1 and stdDevFlag ne 1 and ~(nullExtra) then begin
                for m=0, extraParam-1 do begin
                  formulaValue=doFormula(reform(meanTimeSeriesData[k, *, totalDayCount]), extraToApplies[m])
            ...
            doLog, month, year
            doLog, '**************'
            doLog, meanTimeSeriesData[k, *, totalDayCount], extraValues[k, *, totalDayCount]
            doLog, '**************'
          endif
...
monthlyData=computeMonthlyComplexFinalCheckStatFromDaily(operator, roiNo, resSingleStatToApply, destinationPeriodtype, $
  cvParInfos.id, cvParInfos.label, thrParInfos.id, thrLabels, $
  ...
  yGraphicDisplayName, sensorCode, outputDir, utility)
...
histoMainTitlesBinFreqAbsSigmaExtra=monthlyData.histoMainTitlesBinFreqAbsSigmaExtra
histoMainTitlesBinFreqAbsSigmaSingleExtra1=monthlyData.histoMainTitlesBinFreqAbsSigmaSingleExtra[*, *, *, 0]
histoMainTitlesBinFreqAbsSigmaSingleExtra2=monthlyData.histoMainTitlesBinFreqAbsSigmaSingleExtra[*, *, *, 1]
...
reportMonthlyTimeSeriesBinDataSigmaAbsExtra=monthlyData.reportMonthlyTimeSeriesBinDataSigmaAbsExtra
reportMonthlyTimeSeriesBinDataSigmaSingleAbsExtra1=monthlyData.reportMonthlyTimeSeriesBinDataSigmaSingleAbsExtra[*,*,*,*,*,0]
reportMonthlyTimeSeriesBinDataSigmaSingleAbsExtra2=monthlyData.reportMonthlyTimeSeriesBinDataSigmaSingleAbsExtra[*,*,*,*,*,1]
...
fullFileNameHBinFreqSigmaAbsExtra=monthlyData.fullFileNameHBinFreqSigmaAbsExtra
fullFileNameHBinFreqSigmaSingleAbsExtra1=monthlyData.fullFileNameHBinFreqSigmaSingleAbsExtra[*, *, *, 0]
fullFileNameHBinFreqSigmaSingleAbsExtra2=monthlyData.fullFileNameHBinFreqSigmaSingleAbsExtra[*, *, *, 1]
...
if keyword_set(histoFlag) then begin
  ...
  for jjj=0, 1 do begin
    for jj=0, sigmaStatNum-1 do begin
      for j=0, cvNum-1 do begin
        ...
        tReportAux=reportMonthlyTimeSeriesBinDataFlagAbs[*,*,*,0]
        storeTickNames, tReport, /YAXIS, yTicksNo=yTicksNo, /SUPPRESS_LAST, formatTicks=formatTicks
  ...
  for jj=0, sigmaStatNum-1 do begin
    for j=0, cvNum-1 do begin
      fullFileName=fullFileNameHBinFreqSigmaAbsExtra[jj, j]
      
      ;ignore stats number gt 1, at this moment
      tReport=reportMonthlyTimeSeriesBinDataSigmaAbsExtra[*,*,*,j];, jjj]
      tReportAux=reportMonthlyTimeSeriesBinDataFlagAbs[*,*,*,0]
      storeTickNames, tReport, /YAXIS, yTicksNo=yTicksNo, /SUPPRESS_LAST, formatTicks=formatTicks
      
      cgPS_Open, fullFileName
      mainTitle=histoMainTitlesBinFreqAbsSigmaExtra[jj, j]
      drawSideTitle, mainTitle, reservedArea, COLOR='Black', ALIGNMENT=0.5, CHARSIZE=labelCharSize, ORIENTATION=90
      for i=roiNo, 1, -1 do begin
        reformDims=size(tReport, /DIM)
        thisTSData=reform(tReport[i-1, *, *], reformDims[1:*])
  ...
  for jj=0, sigmaStatNum-1 do begin
    for jjj=0, n_elements(thrParInfos)-1 do begin
      for j=0, cvNum-1 do begin
        fullFileName=fullFileNameHBinFreqSigmaSingleAbsExtra1[jj, j, jjj]
        
        ;ignore stats number gt 1, at this moment
        tReport=reportMonthlyTimeSeriesBinDataSigmaSingleAbsExtra1[*,*,*,j,jjj];, jjj]
        tReportAux=reportMonthlyTimeSeriesBinDataFlagAbs[*,*,*,0]
        storeTickNames, tReport, /YAXIS, yTicksNo=yTicksNo, /SUPPRESS_LAST, formatTicks=formatTicks
        
        cgPS_Open, fullFileName
        mainTitle=histoMainTitlesBinFreqAbsSigmaSingleExtra1[jj, j, jjj]
        drawSideTitle, mainTitle, reservedArea, COLOR='Black', ALIGNMENT=0.5, CHARSIZE=labelCharSize, ORIENTATION=90
        for i=roiNo, 1, -1 do begin
          reformDims=size(tReport, /DIM)
          thisTSData=reform(tReport[i-1, *, *], reformDims[1:*])
  ...
  for jj=0, sigmaStatNum-1 do begin
    for jjj=0, n_elements(thrParInfos)-1 do begin
      for j=0, cvNum-1 do begin
        fullFileName=fullFileNameHBinFreqSigmaSingleAbsExtra2[jj, j, jjj]

        ;ignore stats number gt 1, at this moment
        tReport=reportMonthlyTimeSeriesBinDataSigmaSingleAbsExtra2[*,*,*,j,jjj];, jjj]
        tReportAux=reportMonthlyTimeSeriesBinDataFlagAbs[*,*,*,0]
        storeTickNames, tReport, /YAXIS, yTicksNo=yTicksNo, /SUPPRESS_LAST, formatTicks=formatTicks

        cgPS_Open, fullFileName
        mainTitle=histoMainTitlesBinFreqAbsSigmaSingleExtra2[jj, j, jjj]
        drawSideTitle, mainTitle, reservedArea, COLOR='Black', ALIGNMENT=0.5, CHARSIZE=labelCharSize, ORIENTATION=90
        for i=roiNo, 1, -1 do begin
          reformDims=size(tReport, /DIM)
          thisTSData=reform(tReport[i-1, *, *], reformDims[1:*])
