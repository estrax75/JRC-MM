Pro call_mean_3,  daysNumber, data_in, mean_field, std_mean, std_field, nfield
  ;
  if n_elements(dims) ne 2 then dims=[7200,3600]
  mean_field=fltarr(nfield,dims[0],dims[1])
  ;
  buf1=fltarr(dims[0],dims[1])
  buf2=fltarr(dims[0],dims[1])
  ;

  if nfield eq 3 then buf3=fltarr(dims[0],dims[1])
  ;
  ;tt=size(data_in.day)
  tt=[0,daysNumber]
  ;
  num_used=bytarr(dims[0],dims[1])
  one=num_used
  one(*,*)=1
  ;
  ;
  ...
  ;
  std_field=fltarr(nfield,tt(1),7200,3600)
  std_field(*,*,*,*)=-1.0
  ;
  ;
  if nfield eq 3 then begin
    for t=0, tt(1)-1 do begin
      buf1=fltarr(dims[0],dims[1])
      buf2=fltarr(dims[0],dims[1])
      buf3=fltarr(dims[0],dims[1])
      buf1(*,*)=-1.0
      buf2(*,*)=-1.0
      buf3(*,*)=-1.0
      buf=fltarr(dims[0],dims[1])
      buff=fltarr(dims[0],dims[1])
      ;
      buf(*,*)=data_in(t).red(*,*)
      buff(*,*)=mean_field(0,*,*)
    ...
    for t=0, tt(1)-1 do begin
      ...
      buf(*,*)=data_in(t).red
      buff(*,*)=mean_field(0,*,*)
      idx=where(buf gt 0. and buf lt 1.0 and buff gt 0. and buff lt 1. and data_in(t).flag eq 4.)
      if idx(0) ge 0 then buf1(idx) = abs(buf(idx)-buff(idx))
      ;
      buf=data_in(t).nir
      buff(*,*)=mean_field(1,*,*)
  ...
  ;
  std_mean=fltarr(nfield,dims[0],dims[1])
  ;
  num_used_1=bytarr(dims[0],dims[1])
  num_used_2=bytarr(dims[0],dims[1])
  if nfield eq 3 then num_used_3=bytarr(dims[0],dims[1])
  ;
  buf=fltarr(dims[0],dims[1])
  ;
  buf1=fltarr(dims[0],dims[1])
...
PRO FindEuclideanMatricDistance, daysNumber, data_in, idx_third, distance, mean_field, std_mean, nfield
  ;
  ;
  ;
  ; inputs:
  ...
  ;
  ;
  call_mean_3, daysNumber, data_in,  mean_field,  std_mean, std_field, nfield
  ;
