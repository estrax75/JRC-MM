PRO run_split_plot_time_series_perc_mr_D, request, NODISPLAY=NODISPLAY
  ...
  bins=float(mainApp->getKeyValue('PIXELS_VALID_THRESHOLDS'))
  cvs=float(mainApp->getKeyValue('COEFFICIENT_V_THRESHOLDS'))
...
reportTimeSeriesData=fltarr(roiNo, numInPar,  totDaysNumber, flagStatNum+sigmaStatNum)
statTimeSeriesData=fltarr(roiNo, numInPar,  totDaysNumber, 2)
...
reportMonthlyTimeSeriesBinDataFlagPerc=fltarr(roiNo, numInPar,  monthlyBinsNumber, binNum, flagStatNum)
reportMonthlyTimeSeriesBinDataSigmaPerc=fltarr(roiNo, numInPar,  monthlyBinsNumber, cvNum)
...
fullFileNameHBinFreqFlagPerc=strarr(flagStatNum, binNum)
fullFileNameHBinFreqSigmaPerc=strarr(sigmaStatNum, cvNum)

fullFileName=operator->buildOperatorResultFileName(periodType, parName, monthList, yearList, singleStatToApply, '', outputDir, INTERVAL=INTERVAL, /FULLPATH)
for statIndex=0, flagStatNum-1 do Begin
  for binIndex=0, binNum-1 do begin
    prefix='flagged_cases_perc_pixs_le_'+strcompress(binThresholds[binIndex], /REMOVE)
    fullFileNameHBinFreqFlagPerc[statIndex, binIndex]=operator->buildOperatorResultFileName(periodType, prefix+parName, monthList, yearList, singleStatToApply, '', outputDir+path_sep()+'histo'+path_sep()+'perc', INTERVAL=INTERVAL, /FULLPATH)
  endfor
endfor

for statIndex=0, sigmaStatNum-1 do Begin
  for cvIndex=0, cvNum-1 do begin
    prefix='inhomogeneus_cases_cv_le_'+string(format='(f3.1)', cvThresholds[cvIndex])
    fullFileNameHBinFreqSigmaPerc[statIndex, cvIndex]=operator->buildOperatorResultFileName(periodType, prefix+parName, monthList, yearList, singleStatToApply, '', outputDir+path_sep()+'histo'+path_sep()+'perc', INTERVAL=INTERVAL, /FULLPATH)
  endfor
endfor
          ...
          if outPercentage gt compareValidityThreshold then begin
            ...
            for l=0, numInPar-1 do begin
              if ptr_valid(bands[l]) then begin
                ...
                reportTimeSeriesData[k, l, totalDayCount,0]=Report.invalid_count
...
histoMainTitlesBinFreqPercFlag=strarr(flagStatNum, binNum)
histoMainTitlesBinFreqPercSigma=strarr(sigmaStatNum, cvNum)
  ...
  for l=0, numInPar-1 do begin
    ...
    for m=0, yearNo-1 do begin
      ...
      for n=0, monthNo-1 do begin
        ...
        monthDays=utility->calcDayOfMonth([fix(year),fix(month),1,0])
        for statIndex=0, flagStatNum-1 do begin
          dataToAnalyze=reportTimeSeriesData[k, l, dayCount:dayCount+monthDays-1,flagTypeStatIndexes[statIndex]]
          for binIndex=0, binNum-1 do begin
            bin1=where(dataToAnalyze le binThresholds[binIndex], count)
            ;reportMonthlyTimeSeriesBinDataAbs[k, l, timeIndex,binIndex,statIndex]=count
            reportMonthlyTimeSeriesBinDataFlagPerc[k, l, timeIndex,binIndex,statIndex]=1.*count/monthDays*100
            histoMainTitlesBinFreqPercFlag[statIndex, binIndex]='Flagged cases [%] (case '+strcompress(binThresholds[binIndex], /REMOVE)+')'
            ;fullFileNameHBinFreqPerc[binIndex,statIndex]='flagged_cases_perc_pixs_le_'+strcompress(binsize[binIndex], /REMOVE)
          endfor
          countFileDays=foundTimeSeriesData[k, l, dayCount:dayCount+monthDays-1]
          foundMonthlyTimeSeriesData[k, l, timeIndex]=total(countFileDays, /NAN)
          doLog, 'files found:', dayCount, dayCount+monthDays-1, k, l, '---->', total(countFileDays, /NAN), LEVEL=1
        endfor
        for statIndex=0, sigmaStatNum-1 do begin
          dataToAnalyze=reportTimeSeriesData[k, l, dayCount:dayCount+monthDays-1,sigmaTypeStatIndexes[statIndex]]
          for cvCheckIndex=0, cvNum-1 do begin
            histoMainTitlesBinFreqPercSigma[statIndex, cvCheckIndex]='Inhomogeneus cases [%] (case cv lt '+string(format='(f3.1)', cvThresholds[cvCheckIndex])+')'
            dTA=statTimeSeriesData[k, l, dayCount:dayCount+monthDays-1,0]
            dTACheck=statTimeSeriesData[k, l, dayCount:dayCount+monthDays-1,1]
            cc=where(dta le cvThresholds[cvCheckIndex] and dTACheck eq 1, countCVDays)
            reportMonthlyTimeSeriesBinDataSigmaPerc[k, l, timeIndex, cvCheckIndex]=(1.*countCVDays/monthDays)*100
          endfor
        ...
        if foundMonthlyTimeSeriesData[k, l, timeIndex] eq 0 then begin
          ...
          reportMonthlyTimeSeriesBinDataSigmaPerc[k, l, timeIndex, *]=!VALUES.F_NAN
          reportMonthlyTimeSeriesBinDataFlagPerc[k, l, timeIndex, *]=!VALUES.F_NAN
        endif
...
if keyword_set(histoFlag) then begin
  ...
  refMin=min(reportMonthlyTimeSeriesBinDataFlagPerc, max=refMax, /NAN)
  ;reportMonthlyTimeSeriesBinDataFlagPerc
  rMax=refMax
  refMax=refMax>100
  ...
  for jj=0, flagStatNum-1 do begin
    for j=0, binNum-1 do begin
      fullFileName=fullFileNameHBinFreqFlagPerc[jj, j]
      ;ignore statNumber gt 1...
      tReport=reportMonthlyTimeSeriesBinDataFlagPerc[*,*,*,j]
      storeTickNames, tReport, /YAXIS, yTicksNo=yTicksNo, /SUPPRESS_LAST, formatTicks=formatTicks
      
      cgPS_Open, fullFileName
      mainTitle=histoMainTitlesBinFreqPercFlag[jj, j]
      ...
      for i=roiNo, 1, -1 do begin
        ...
        numberOfData='(N= '+string(format='(i6)', fileNo)+')'
  ...
  refMin=min(reportMonthlyTimeSeriesBinDataSigmaPerc, max=refMax, /NAN)
  rMax=refMax
  refMax=refMax>100 ;or 100% fix...
  yScaleRange=[(0<refMin), refMax+refMax/10]
  order=strcompress(ceil(alog(rMax)), /REMOVE)
  ;YTICKVALUES=string([yScaleRange[0], rMax], format='(i'+order+'.0)')
  ...
  for jj=0, sigmaStatNum-1 do begin
    for j=0, cvNum-1 do begin
      fullFileName=fullFileNameHBinFreqSigmaPerc[jj, j]
      
      ;ignore stats number gt 1, at this moment
      tReport=reportMonthlyTimeSeriesBinDataSigmaPerc[*,*,*,j]
      storeTickNames, tReport, /YAXIS, yTicksNo=yTicksNo, /SUPPRESS_LAST, formatTicks=formatTicks
      
      cgPS_Open, fullFileName
      mainTitle=histoMainTitlesBinFreqPercSigma[jj, j]
      drawSideTitle, mainTitle, reservedArea, COLOR='Black', ALIGNMENT=0.5, CHARSIZE=labelCharSize, ORIENTATION=90
      for i=roiNo, 1, -1 do begin
        reformDims=size(tReport, /DIM)
        thisTSData=reform(tReport[i-1, *, *], reformDims[1:*])
