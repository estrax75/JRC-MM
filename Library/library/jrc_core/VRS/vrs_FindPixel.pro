...
; PURPOSE:
;
;       find pixel numbers in a VIIRS image for the pixel nearest to a specific location 
...
; CALLING SEQUENCE:
;
;       FindPixel,SatStruct,latitude,longitude,inlat,inlon,n_side,near_line,near_elem,istatus
;
; INPUTS:
;			SatStruct: 		structure defining the entire SeaWiFS image
;			latitude:		latitude field of the image
;			longitude:		longitude field of the image
...
;			startline:			first line number for extraction.
;			endline:			last line number for extraction.
...
;
; COMMENTS:
;		input structure provided by routine GetVRSInfo
;			
; REFERENCES:
;
; EXAMPLE:
...
PRO vrs_FindPixel,SatStruct,latitude,longitude,inlat,inlon,n_side,near_line,near_elem,istatus
...
; First search on a subsampled grid
   FOR elem=0,nelem-1,10 DO BEGIN
      ...
      this_lat = DOUBLE( latitude(elem,line) )
      this_lon = DOUBLE( longitude(elem,line) )
      
      dlat = DOUBLE( ABS(inlat-this_lat) ) & dlon = DOUBLE(ABS(inlon-this_lon) )
      
      IF ( dlat LT  latthreshold AND dlon LT  lonthreshold ) THEN BEGIN
   
          dist_from_point_sq = dlon*dlon + dlat*dlat ; simple equation for now
