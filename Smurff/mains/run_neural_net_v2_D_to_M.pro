PRO run_neural_net_v2_D_to_M, request, NODISPLAY=NODISPLAY
  ...
  for i=0, n_elements(yearList)-1 do begin
    for j=0, n_elements(monthList)-1 do begin
      for k=0, n_elements(roiArchiveList)-1 do begin
        ...
        res = do_single_neural_net_v2_D_to_M(periodType, monthList[j], yearList[i], roiCodeList[k], roiArchiveList[k], $
