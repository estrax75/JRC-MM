...
function createLinearColorScaleInfo, colorNumber, lowVal, highVal

  COMMON smurffCB, mainApp
  
  newLow=lowVal
  newHigh=highVal
  trueValues=findgen(colorNumber)*((newHigh-newLow)/colorNumber)+newLow
  ...
  labelValues=buildLinearValues(lower, higher)
  ...
  markValues=[0.]
  ;markValues=fltarr(2*count)
  for i=0, count-2 do markValues=[markValues, (labelValues[i]+labelValues[i+1])/2]
  markValues=[markValues, (labelValues[count-1]+highVal)/2]
