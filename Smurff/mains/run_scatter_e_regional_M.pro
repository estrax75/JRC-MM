PRO run_scatter_e_regional_M, request, NODISPLAY=NODISPLAY
  ...
  if periodType ne 'M' then message, 'This run is designed for working with monthly (empirical regional) data, you need to (copy and) custom it if you want a different interval period and/or data'
    ...
    for j=0, n_elements(monthList)-1 do begin
      ...
      for k=0, n_elements(roiCodeList)-1 do begin
        ...
        data1 = call_function(readFunctionList[0]+'_'+periodType, periodType, monthList[j], yearList[i], roiCodeList[k], roiArchiveList[k], inputDirs[0], outputDir, $
          varList[0], NOTFOUND=NOTFOUND, refRoi=refRoiCode, GETCHLVAR='ALG_CHL' eq varList[0])
        ...
        if ~keyword_set(NOTFOUND) then data2 = call_function(readFunctionList[1]+'_'+periodType, periodType, monthList[j], yearList[i], roiCodeList[k], roiArchiveList[k], inputDirs[1], outputDir, $
          varList[1], NOTFOUND=NOTFOUND, refRoi=refRoiCode, GETCHLVAR='ALG_CHL' eq varList[1]) else continue
        if ~keyword_set(NOTFOUND) then begin
          xTitle=labelList[0]
          yTitle=labelList[1]
          xy=size(data1, /DIM)
          ...
          title=monthList[j]+' - '+year+' - '+roiInfo.displayName+' - '+'JRC Vs ' + physical->getDisplayName()
          ...
          res=call_function('do_single_scatter'+'_'+periodType ,dataList, $
            ...
            title, xtitle, ytitle, $
            overwriteFlag=overwriteFlag, deleteInputFlag=deleteInputFlag, day=day, outParCode=varList[0], $
