PRO run_e_regional_D_file_to_M, request, NODISPLAY=NODISPLAY
  ...
  for i=0, n_elements(yearList)-1 do begin
    for j=0, n_elements(monthList)-1 do begin
      for k=0, n_elements(roiArchiveList)-1 do begin
        ...
        res = do_single_e_regional_D_file_to_M(periodType, monthList[j], yearList[i], roiCodeList[k], roiArchiveList[k], $
        ...
        if keyword_set(NOTFOUND) then doLog,'skip file month: ', monthList[j], ' year: ', yearList[i], ' roi: ', roiArchiveList[k], level=2
