;FUNCTION computematchup_v5, flagData, chlaData, angData, aotData,  sunZenithData, rrs443Data,rrs490Data, rrs555Data, flagDataOriginal
FUNCTION computeMatchUp_v5, flagToBit, flagData, $
  ...
  if matchup4a eq 0 then begin
    if mChla le meanThresholds[0] and mAng le meanThresholds[1] and mAot le meanThresholds[2] and mSolz le meanThresholds[3] then matchUp4a=0 else matchUp4a=1
  endif
  ...
  if matchup4b eq 0 then begin
    if mChla le meanThresholds[0] and mAng le meanThresholds[1] and mAot le meanThresholds[2] and mSolz le meanThresholds[3] then matchUp4b=0 else matchUp4b=1
  endif
  ...
  if matchup8a eq 0 then begin
    if mSolz le meanThresholds[3] then matchUp8a=0 else matchUp8a=1
  endif
  ...
  if matchup8b eq 0 then begin
    if mSolz le meanThresholds[3] then matchUp8b=0 else matchUp8b=1
  endif
  ...
  return, {desc:[matchUpDesc1,matchUpDesc2a,matchUpDesc2b,matchUpDesc3a,matchUpDesc3b,matchUpDesc4a,matchUpDesc4b,$
    ...
    statParameters: ptr_new(parList), $
    statValues:[ptr_new(meanList), ptr_new(stdDevList)] $
    }
