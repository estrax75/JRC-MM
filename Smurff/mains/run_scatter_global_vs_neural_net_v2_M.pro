PRO run_scatter_global_vs_neural_net_v2_M, request, NODISPLAY=NODISPLAY
    ...
    for j=0, n_elements(monthList)-1 do begin
      ...
      for k=0, n_elements(roiCodeList)-1 do begin
        ...
        if ~keyword_set(NOTFOUND) then begin
          ...
          title=monthList[j]+' - '+year+' - '+roiInfo.displayName+' - '+'Global Vs ' + 'Neural Net V2';physical->getDisplayName()
