PRO computeSingleSatelliteExtraction_v2
  ...
  ;siteList=['NPO','MSea', 'CSea', 'ASea', 'NAO', 'LSea', 'EIO', 'SoS', 'BSea']
  siteList=['SPG']
  ;siteList=['SoS', 'BSea']
  ;siteList=['EIO', 'ASea']
  ;siteList=['NPO']
  ...
  statinDir='E:\projects\idl\smurff\extractStat\diagnostic\last\data\input\'
  statOutDir='E:\projects\idl\smurff\extractStat\diagnostic\last\data\output\'
  ...
  for i=0, n_elements(sensorList)-1 do begin
    ...
    for j=0, n_elements(siteList)-1 do begin
      thisSite=siteList[j]
      fileToRead=statinDir+'satInfo'+'_'+thisSensor+'_'+thisSite+'.sav'
      ...
      for k=0, count-1 do begin
        ...
        check=file_info([flagFile, chlafile, angFile, aotFile, rrs443File, rrs490File, rrs555File])
        if total(check.size) eq 0 then begin
          print, 'skip-->', flagFile 
          continue
        endif
        ...
        matchUPStruct=computeMatchUp_v2(flagToBit, angData, aotData, chlaData, rrs443Data,rrs490Data, rrs555Data)
      ...
      advancedInfoFileName=statoutDir+'advSatInfo'+'_'+thisSensor+'_'+thisSite+'.sav'
  ...
  ;save, summaryStats, fileName=statoutDir+'summary.sav'
