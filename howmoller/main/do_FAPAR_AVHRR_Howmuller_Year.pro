...
PRO do_FAPAR_AVHRR_Howmuller_Year, year, sourceParameter, parameter, parSelector, inputDir, outputDir, noaanumber, $
  ...
  ;n_elements(first)
  if parameter eq 'FAPAR' then maxk=0 else maxk=3
  ...
  for k=0, maxk do begin
    ...
    if parSelector eq 'FAPAR' then begin
      FAPARCOLOR
      titlefig=['FAPAR', 'Unknown'];titlefig=['FAPAR','2 x Sigma']
      namefig=['FAPAR', 'Unknown'];namefig=['FAPAR', 'SigmaFAPAR']
      pencolor=255
    endif

    set_plot,'ps'

    fname=getHowMollerSavFile(namefig[k], year, undefined, 'YEAR', noaanumber, sourceParameter, parSelector)
    ...
    if parSelector eq 'BRF' or parSelector eq 'FAPAR' then lev=[0.01,0.02,0.05,0.07, 0.1,0.15,0.2,0.25,0.30,0.40,0.50,0.60,0.70,0.80,0.90,0.95, 1.00] else $
      lev=[0.01,0.02,0.05,0.07, 0.1,0.15,0.2,0.25,0.30,0.40,0.50]
    if parSelector eq 'BRF' or parSelector eq 'FAPAR' then xnam=[' ','.01',' ','.02',' ','.05',' ','.07',' ','0.10',' ','.15',' ','.20',' ',$
    ...
    ;	nummean(l,1)=sdev
    ;endfor
    if parSelector eq 'BRF' or parSelector eq 'FAPAR' then cc_color=[2,20,30,40,55,60,70,80,100,110,130,150,170,190,210,235,256] else cc_color=lev*500.0
    time=findgen(i)
