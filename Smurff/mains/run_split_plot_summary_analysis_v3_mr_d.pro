PRO run_split_plot_summary_analysis_v3_mr_d, request, NODISPLAY=NODISPLAY
        ...
        for duplicate=0, 1 do begin
          for k=0, roiNo-1 do begin
            ...
            for l=0, numInPar-1 do begin
              ...
              if ~keyword_set(NOTFOUND) then begin
                histoMainTitlesCheck=extraDataInfo;.plotTitles
            ...
            if dataOK eq numInPar then begin
              ...
              for l=0, numInPar-1 do begin
                if ptr_valid(bands[l]) then begin
                  ...
                  lastDim=thisDim
                  band=*bands[l]
                  totTimeSeriesData[k, l, subscribeTotalDayCount]=band
            ...
            totFound[k]=totFound[k]+addFoundCount
          endfor
  ...
  monthlyData=computeMonthlyFromDaily(operator, roiNo, resSingleStatToApply, destinationPeriodtype, $
    yearList, monthList, $
    cvTimeSeriesData, foundTimeSeriesData, $
    sensorCode, outputDir, utility, /DUPLICATE)

  ;fullFileNameHistoCheck=indgen()
  ...
  if keyword_set(histoFlag) then begin
    ...
    refMin=min(reportMonthlyTimeSeriesDataCheck3, max=refMax, /NAN)
    rMax=refMax
    refMax=refMax>31
    yScaleRange=[(0<refMin), refMax+refMax/10]
    order=strcompress(ceil(alog(rMax)), /REMOVE)
    ...
    refMin=min(reportMonthlyTimeSeriesDataCheck4, max=refMax, /NAN)
    rMax=refMax
    refMax=refMax>31
    yScaleRange=[(0<refMin), refMax+refMax/10]
    order=strcompress(ceil(alog(rMax)), /REMOVE)
    ...
    refMin=min(reportMonthlyTimeSeriesDataCheck5, max=refMax, /NAN)
    rMax=refMax
    refMax=refMax>31
    yScaleRange=[(0<refMin), refMax+refMax/10]
    order=strcompress(ceil(alog(refMax)), /REMOVE)
    ;YTICKVALUES=string([yScaleRange[0], rMax], format='(i'+order+'.0)')
    YTICKVALUES=string([yScaleRange[0], refMax], format='(i'+order+'.0)')
    YTICKFORMAT='getVoidTickName'
    formatTicks='(i'+order+'.0)'
    ...
    refMin=min(reportMonthlyTimeSeriesDataCheck6, max=refMax, /NAN)
    rMax=refMax
    refMax=refMax>31
    yScaleRange=[(0<refMin), refMax+refMax/10]
    order=strcompress(ceil(alog(rMax)), /REMOVE)
    ...
    refMin=min(reportMonthlyTimeSeriesDataCheck7, max=refMax, /NAN)
    rMax=refMax
    refMax=refMax>31
    yScaleRange=[(0<refMin), refMax+refMax/10]
    order=strcompress(ceil(alog(rMax)), /REMOVE)
    ...
    j=0
    for jj=0, sigmaStatNum-1 do begin
      for jjj=0, trueFlagNo-1 do begin
        ;for j=0, cvNum-1 do begin
