function getSensorCoeffs, sensor, sensorCode, SOIL=SOIL, MISSIONOVERLAPINDEX=MISSIONOVERLAPINDEX, coeffFile=coeffFile

  coeffInfo=getCoeffStruct()

  if n_elements(coeffFile) eq 1 then begin
    fInfo=file_info(coeffFile)
    if fInfo.size gt 10 and fInfo.read eq 1 then begin
      fileCoeff=mapCoeff(coeffFile)
      coeffInfo.G0coeffs=[fileCoeff.g0Coeff1,fileCoeff.g0Coeff2,fileCoeff.g0Coeff3,fileCoeff.g0Coeff4,fileCoeff.g0Coeff5,fileCoeff.g0Coeff6]

      ; MM 20160506 coeffs from table 8 pag. 28
      ; k, theta, rho
      if  keyword_set(SOIL) then begin
        coeffInfo.RahmanCoeffs_RED =[fileCoeff.baresoilkiband1,fileCoeff.baresoilThetahgBand1,fileCoeff.baresoilRhoicBand1]
        coeffInfo.RahmanCoeffs_NIR =[fileCoeff.baresoilkiband2,fileCoeff.baresoilThetahgBand2,fileCoeff.baresoilRhoicBand2]
      endif else begin
        coeffInfo.RahmanCoeffs_RED =[fileCoeff.kiband1,fileCoeff.ThetahgBand1,fileCoeff.RhoicBand1]
        coeffInfo.RahmanCoeffs_NIR =[fileCoeff.kiband2,fileCoeff.ThetahgBand2,fileCoeff.RhoicBand2]
      endelse

      ; MM 20160531 coeffs from table 8 pag. 28
      coeffInfo.soilCoeffs=[fileCoeff.bareSoilsAs,fileCoeff.bareSoilsBs]
    endif else begin
      message, 'wrong or mistyped coeffFile: '+coeffFile
    endelse
  endif else begin
    CASE sensor OF
      'SEA' : BEGIN
