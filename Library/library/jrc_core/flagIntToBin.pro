function flagIntToBin, intFlag, NOFLAG=NOFLAG

  ;dims=size(intFlag, /dimensions)
  ;lastDim=dims[n_elements(dims)-1]
  res=ptrarr(32)
  
  ;bitArr=ptrarr(n_elements(intFlag))
  ;bitsMatrix=bytarr(lastDim)
  
  ;for k=0,  lastDim-1 do begin
  NOFLAG=1
  if total(intFlag) ne 0 then begin
    NOFLAG=0
    newValues=intFlag
    FOR j=0,31 DO BEGIN
      ;powerOfTwo = 2ul^j
      ;res[j]=ptr_new(total(intFlag mod powerOfTwo), /NO_COPY)
      bitValues=newValues mod 2
      res[j]=ptr_new(bitValues, /NO_COPY)
