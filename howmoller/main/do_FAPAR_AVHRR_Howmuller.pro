pro do_FAPAR_AVHRR_Howmuller, month, year, sourceParameter, parameter, formatType, inputBaseDir, outputBaseDir, missionIndex, $
  ...
  ;restorefileName='/home/mariomi/data/HM/FAPAR_199303_DAILY_avhrr_AVHRR11.sav'
  parSelector='FAPAR'
    ...
    for timecount=0, timeLapse-1 do begin
      ...
      if  thisFile.found then begin
        ...
        if strupcase(formatType) eq 'NC' then begin
          ncdfread,  thisFile.fullfilename,parListToRead[0], fapar, slope_f, intercept_f, dims
          ncdfread,  thisFile.fullfilename,parListToRead[1], ldtr, slope_ldtr, intercept_ldtr, dims
        ...
        if strupcase(formatType) eq 'HDF' then begin
          ST_operator->readHdfFullInfoData, thisFile.fullfilename, parListToRead[0], fapar, slope_f, intercept_f, f_fillvalue, ERROR=ERROR, /APPLY
          ST_operator->readHdfFullInfoData, thisFile.fullfilename, parListToRead[1], ldtr, slope_ldtr, intercept_ldtr, dims, ERROR=ERROR
        endif
        ...
        for lon=0, 3600-1 DO BEGIN
          bug1=float(fapar(*,lon))*slope_f+intercept_f
  ...
  if parSelector eq 'FAPAR' then begin
    FAPARCOLOR
    titlefig=['FAPAR'];titlefig=['FAPAR','2 x Sigma']
    namefig=['FAPAR'];namefig=['FAPAR', 'SigmaFAPAR']
    pencolor=254;pencolor=0
  endif
  ...
  titlefig=['Fapar']
  namefig=['Fapar'];namefig=['Band_1', 'Band_2', 'Sigma_Band_1', 'Sigma_Band_2']
  ...
  for i=0, 0 do begin
    ...
    set_plot,'ps'
    epsFileName=getHowMollerEpsFile(parameter, namefig[i], '_Global_lat_daily_fapar_avhrr_', month, year, noaanumber)
