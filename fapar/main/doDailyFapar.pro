...
;@../../Library/library/jrc_core/mapQualityFlags
function doDailyFapar, instrument, indicator, spatialResolution, level, missionName, mainVarName, missionCode, year, month, day, $
  sourceDir, outputDir, tempdir, $
  FIRST_LOOK=FIRST_LOOK, $
  NC=NC, HDF=HDF, CSV=CSV, MISSIONOVERLAPINDEX=MISSIONOVERLAPINDEX, $
  PIXELS_PROCESS=PIXELS_PROCESS, $
  OVERWRITE=OVERWRITE, subfolder=subfolder, DISKSPACE=DISKSPACE, $
  coeffFile=coeffFile
  ...
  if ~(keyword_set(PIXELS_PROCESS)) then begin
    ...
    checkNCzip1=file_info(resFileNC+'.gz')
    checkNCzip2=file_info(resFileNC+'.zip')
    checkNCzip=checkNCzip1
    checkNCzip.size=checkNCzip1.size > checkNCzip2.size

    checkHDFzip1=file_info(resFileHDF+'.gz')
    checkHDFzip2=file_info(resFileHDF+'.zip')
    checkHDFzip=checkHDFzip1
    checkHDFzip.size=checkHDFzip1.size > checkHDFzip2.size
    ...
    ; force write on 'data'
    if keyword_set(DISKSPACE) then begin
      destDir='/data/storage/working/'+hdfFileInfo.filePath
    ...
    ; reading input data
    data=read_BRF(sourceDir, sourceFileInfo.fileName, FOUND=FOUND)
    print, 'brf ...done'
    dimensions=size(data.red_avhrr, /DIMENSION)
  endif else begin
    ; read csv file
    ;fileType='csv'
    ;version='V01'
    ...
    print, 'working on', year, month, day, '(', yearDay, ')', '...'
    fileType='tsv'

    sourceDir=ST_fileSystem->adjustDirSep(sourceDir)
    sourceFile=build_JRC_SPP_AVH_Daily_FileName(instrument, missionCode, year, month, yearDay, sourceDir, fileType, MISSIONNUMBER=missionCode, /JULDAY);, /YEARFOLDER)

    ;sourceFileInfo=build_JRC_PixelProcessor_AVH_Daily_Product_FileName(instrument, year, month, day, timestamp, temporalResolution, location, spatialResolution, $
    ;  product, version, fileType,  indicator=indicator, level, projection=projection)
    destFileInfoNC=build_JRC_FPA_AVH_Daily_Product_FileName(instrument, year, month, day, timestamp, temporalResolution, location, spatialResolution, $
      product, version, 'nc',  indicator=indicator, level, projection=projection)
    destFileInfoCSV=build_JRC_FPA_AVH_Daily_Product_FileName(instrument, year, month, day, timestamp, temporalResolution, location, spatialResolution, $
      product, version, 'csv',  indicator=indicator, level, projection=projection)
    ;;
    destDir=outputDir+destFileInfoCSV.filePath
    destDir=ST_fileSystem->adjustDirSep(sourceDir, /ADD)
    sourceDir=ST_fileSystem->adjustDirSep(sourceDir, /ADD)
    ;resFileCSV=destDir+destFileInfo.fileName

    sourceFullFileName=file_search(sourceDir[0], sourceFile[0], count=count)
    fInfo=file_info(sourceFullFileName)
    if fInfo.size gt 10 then begin
      count = 1
      sourceFile=ST_fileSystem->getFileNameInfo(sourceFullFileName, filePath=filePath, extension=extension)
    endif
    resFileCSV = destDir+destFileInfoCSV.fileName
    resFileNC = destDir+destFileInfoNC.fileName
    checkCSV=file_info(resFileCSV)
    checkNC=file_info(resFileNC)

    ; search file existence...

    if count ne 1 then NOSOURCEAVAILABLE=1
    if keyword_set(CSV) and ((checkCSV[0].size eq 0)  or keyword_set(OVERWRITE)) then NOWRITECSV=0 else NOWRITECSV=1
    if keyword_set(NC) and ((checkNC[0].size eq 0)  or keyword_set(OVERWRITE)) then NOWRITENC=0 else NOWRITENC=1

    ; do the job ONLY if we need it!!! Otherwise, skip
    if keyword_set(NOSOURCEAVAILABLE) or keyword_set(NOWRITECSV) then return, -1

    ;;
    FOUND=0
    data=read_BRF(sourceDir, sourceFile, FOUND=FOUND, PIXELS_PROCESS=PIXELS_PROCESS, CSV=CSV, NC=NC, origDims=origDims, extraData=extraData)
    dimensions=size(data.red_avhrr, /DIMENSION)
  endelse
  ...
  ;main structure
  output={  fpar: fltarr(dimensions[0],dimensions[1]), $f
    sigma: fltarr(dimensions[0],dimensions[1]), $
    red: fltarr(dimensions[0],dimensions[1]), $
    sigma_red:fltarr(dimensions[0],dimensions[1]), $
    nir: fltarr(dimensions[0],dimensions[1]), $
    sigma_nir: fltarr(dimensions[0],dimensions[1]), $
    flag: bytarr(dimensions[0],dimensions[1]), $
    toc_red: fltarr(dimensions[0],dimensions[1]), $
    toc_nir: fltarr(dimensions[0],dimensions[1]), $
    qa: bytarr(dimensions[0],dimensions[1])}
  ;
  ; initialization
  ;
  ; MM & NG 21/09/2016
  ...
  qa_avhrr=fix(data.brf_qa_avhrr) & slope_brf_qa=data.slope_qa & offset_brf_qa=data.offset_qa
  ...
  ;
  ; correction already applied (SPP)
  ...
  ;
  ;
  flagstoc= bytarr(dimensions[0], dimensions[1])
  ...
  coeffInfo=getSensorCoeffs(instrument, missionCode, coeffFile=coeffFile)
  ...
  if instrument eq 'AVH' then begin
    ...
    if idx(0) ge 0 then begin
      ...
      ;rhoRED=!VALUES.F_NAN
      ;rhoNIR=!VALUES.F_NAN
      ;D_rhoRED=!VALUES.F_NAN
      ;D_rhoNIR=!VALUES.F_NAN
      ;D_rhotildeBLUE=!VALUES.F_NAN
      ;D_rhotildeRed=!VALUES.F_NAN
      ;D_rhotildeNir=!VALUES.F_NAN
      ;if finite(D_BRF_ToA_RED(idx)) then
      FAPAR,'AVHRR', NOAString,sza(idx),vza(idx),saa(idx),vaa(idx),$
        ...
        MISSIONOVERLAPINDEX=MISSIONOVERLAPINDEX, coeffFile=coeffFile
      ;
      ...
      ;img=fltarr(dimensions[0],dimensions[1])
      ;img(idx)=rhoRED
      ;
      ;	window, 2, xsize=720, ysize=360, title='Rectified Red '+SENSOR
  ...
  if idx_soil(0) ge 0 then begin
    ...
    Rectified,'AVHRR', NOAString,sza(idx_soil),vza(idx_soil),saa(idx_soil),vaa(idx_soil),$
      ...
      D_rhotildeBLUE_S, D_rhotildeRED_S, D_rhotildeNIR_S, vi_s, d_vis, /TOC, coeffFile=coeffFile
    ;;
    ; flag = 3 overwrite
  ...
  if ~keyword_set(PIXELS_PROCESS) then begin
    ...
    ;for i=0, n_elements(flagTags)-1 do begin
    ;  thisIdx=(where(flagTags[i] eq tags, count))[0]
    ;  nanIdxs=where(~finite(output.(thisIdx)) and output.(thisIdx) gt 250, count)
    ...
    if cnt ne 0 then begin
      ;anglesNan=where(tv_avhrr eq -9999, cntAnglesNan)
      ; use F_NAN instead of INT_NAN (we don't remap NaN as INT or BYTE)
      ts[nanIdx]=!VALUES.F_NAN
      tv[nanIdx]=!VALUES.F_NAN
      phi[nanIdx]=!VALUES.F_NAN
      output.sigma_red[nanIdx]=!VALUES.F_NAN
      output.sigma_nir[nanIdx]=!VALUES.F_NAN
      output.red[nanIdx]=!VALUES.F_NAN
      output.nir[nanIdx]=!VALUES.F_NAN
      toc_red[nanIdx]=!VALUES.F_NAN
      toc_nir[nanIdx]=!VALUES.F_NAN
    endif

    faparDSInfo=getStandardFaparDataSetInfo()
    bandNames=faparDSInfo.bandNames
    outPar=n_elements(bandNames)
    resMatrix=strarr(outPar, origDims)
    i=0
    ; ToDo Add original field (lat/lon/Id/Radiance1/Radiance2)
    resMatrix[i,*]=string(reform(output.fpar[0:origDims-1, 0]), format='(F10.8)') & i++
    resMatrix[i,*]=string(reform(output.sigma[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.red[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.sigma_red[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.nir[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.sigma_nir[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(qa_avhrr[0:origDims-1, 0]), format='(I5)') & i++
    resMatrix[i,*]=string(reform(ts[0:origDims-1, 0]), format='(F7.3)') & i++
    resMatrix[i,*]=string(reform(tv[0:origDims-1, 0]), format='(F7.3)') & i++
    resMatrix[i,*]=string(reform(phi[0:origDims-1, 0]), format='(F8.3)') & i++
    resMatrix[i,*]=string(reform(toc_red[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(toc_nir[0:origDims-1, 0]), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.flag[0:origDims-1, 0]), format='(I2)') & i++
    ;;
    extraDim=size(extradata, /DIMEN)
    resMatrix=strarr(outPar+extraDim[0], n_elements(output.fpar))
    bandNames=[bandNames, extradata[*,0]]

    i=0
    resMatrix[i,*]=string(reform(output.fpar), format='(F10.8)') & i++
    resMatrix[i,*]=string(reform(output.sigma), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.red), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.sigma_red), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.nir), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.sigma_nir), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(qa_avhrr), format='(I5)') & i++
    resMatrix[i,*]=string(reform(ts), format='(F7.2)') & i++
    resMatrix[i,*]=string(reform(tv), format='(F7.2)') & i++
    resMatrix[i,*]=string(reform(phi), format='(F8.2)') & i++
    resMatrix[i,*]=string(reform(toc_red), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(toc_nir), format='(F8.6)') & i++
    resMatrix[i,*]=string(reform(output.flag), format='(I2)') & i++
    for k=0, origDims[0]-1 do resMatrix[i:i+extraDim[0]-1,k]=extradata[*,k+1] 
    ;if output.flag[0] ne 0 then stop
    ;print, resMatrix[*,0:(origDims-1)>1]
    write_csv, resFileCSV, resMatrix[*,0:(origDims-1)>1], header=bandNames
    if origDims eq 1 then ST_fileSystem->workAroundCSVFile, resFileCSV  
