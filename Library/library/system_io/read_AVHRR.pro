...
function read_AVHRR, folder, fileName, FOUND=FOUND, $
  FULL=FULL, APPLY_CONVERSION=APPLY_CONVERSION, varName=varName, offset=offset, count=count, fid=fid, $
  PIXELS_PROCESS=PIXELS_PROCESS, HDF=HDF, NC=NC, CSV=CSV, ORIGDIMS=ORIGDIMS
  ...
  if keyword_set(PIXELS_PROCESS) then begin
    if keyword_set(CSV) then allData=readSPP_AVHRR_FromCSV(folder, fileName, FULL=FULL, FOUND=FOUND, ORIGDIMS=ORIGDIMS)
    if keyword_set(NC) then allData=readSPP_AVHRR_FromNC(folder, fileName, FULL=FULL, FOUND=FOUND, ORIGDIMS=ORIGDIMS)
  endif else begin
    allData=readMatrix_AVHRR_FromNC(folder, fileName, varName=varname, offset=offset, count=count, FOUND=FOUND, FULL=FULL, USENAN=USENAN, APPLY_CONVERSION=APPLY_CONVERSION)
  endelse
