function _doBRFLand_switched, sourceFile, confDir, year, month, day, sensor, missionCode, noaaCode, resolution, mainVar, destBaseDir, tempDir, $
  ...
  ;  outputDir=writeBaseDir+resFileNCInfo.filePath
  ...
  ;CAREFULLLLLLLL switch here!!!!!
  infoVar1b=['SREFL_CH2', sourceFile, fName, dir, ext, 'input_data[0,*]/red_avhrr']

  ;ST_operator->readHdfFullInfoData, infoVar1b[1], infoVar1b[0], red_avh09, red_slope, red_offset, red_fillvalue, ERROR=ERROR
  ncdfread,  infoVar1b[1], infoVar1b[0], red_avh09, red_slope, red_offset, dim, red_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  input_data=fltarr(2, globDim)
  ;CAREGFULLLLLLLL switch here!!!!!
  ...
  infoVar2b=['SREFL_CH1', sourceFile, fName, dir, ext, 'input_data[1,*]/nir_avhrr']

  ;CAREFULLLLLLLL switch here!!!!!
  ncdfread, infoVar2b[1], infoVar2b[0], nir_avh09, nir_slope, nir_offset, dim, nir_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  ncdfread, infoVar3b[1], infoVar3b[0], ts_avh09, ts_slope, ts_offset, dim, ts_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  ncdfread, infoVar4b[1], infoVar4b[0], tv_avh09, tv_slope, tv_offset, dim, tv_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  ncdfread, infoVar5b[1], infoVar5b[0], qa_avh09, qa_slope, qa_offset, dim, qa_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  ncdfread, infoVar6b[1], infoVar6b[0], phi_avh09, phi_slope, phi_offset, dim, phi_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ; phi "correction"
  ...
  for j=0, validCount-1 do begin
    ...
    for i=0,1 do begin
      ...
      if ((BRFVs ne 0) and (BRDFRs ne 0)) then begin
        ...
        BRFV = (BRDFVs*ndvi_value) + BRF_params[i].intercept_v1[checkIndex];
        BRFR = (BRDFRs*ndvi_value) + BRF_params[i].intercept_r1[checkIndex];
        ;    print, 'v,r', brfv, brfr
  ...
  savFileName=ST_fileSystem->addFileExtension(destDir+sourceFileName, 'sav')
  ...
  ;BRF_params[i].slope_v1[checkIndex];
  ; check BRF
  ;  b1v=reform(BRF_params[0].slope_v1, 7200, 3600)
  ;  b1r=reform(BRF_params[0].slope_r1, 7200, 3600)
  ;  b2v=reform(BRF_params[1].slope_v1, 7200, 3600)
  ;  b2r=reform(BRF_params[1].slope_r1, 7200, 3600)
  ;  ;b1v[IDX_SEA]=NaN
  ;
  ;  max=max(b1v, min=min)
  ...
  ;  window, 0, xsize=720, ysize=360, title='red brfl: '+SENSOR
  ;  tv, reverse(rebin(bytscl(red_brf, min=0., max=1.0), 720,360),2)
  ;  window, 1, xsize=720, ysize=360, title='nir brfl: '+SENSOR
  ...
  ;
  ;  window, 2, xsize=720, ysize=360, title='nir brfl: '+SENSOR
  ;  tv, reverse(rebin(bytscl(final_nir_avhrr, min=0., max=1.0), 720,360),2)
  ;  window, 3, xsize=720, ysize=360, title='red brfl: '+SENSOR
  ;  tv, reverse(rebin(bytscl(final_red_avhrr, min=0., max=1.0), 720,360),2)
  ;
