...
function doBRF, sourceFile, confDir, year, month, day, sensor, missionCode, noaaCode, resolution, mainVar, outputDir, operatorObj, fsObj, tempDir, $
  ...
  confDir=fsObj->adjustDirSep(confDir, /ADD)
  ...
  resFileNC=fsObj->addFileExtension(filePath+resFName, 'nc')
  ...
  fName=fsObj->getFileNameInfo(sourceFile, filePath=dir, extension=ext)
  ...
  operatorObj->readHdfFullInfoData, infoVar1b[1], infoVar1b[0], red_avh09, red_slope, red_offset, red_fillvalue, ERROR=ERROR
  ...
  operatorObj->readHdfFullInfoData, infoVar2b[1], infoVar2b[0], nir_avh09, nir_slope, nir_offset, nir_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  operatorObj->readHdfFullInfoData, infoVar3b[1], infoVar3b[0], ts_avh09, ts_slope, ts_offset, ts_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  operatorObj->readHdfFullInfoData, infoVar4b[1], infoVar4b[0], tv_avh09, tv_slope, tv_offset, tv_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  operatorObj->readHdfFullInfoData, infoVar5b[1], infoVar5b[0], qa_avh09, qa_slope, qa_offset, qa_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ...
  operatorObj->readHdfFullInfoData, infoVar6b[1], infoVar6b[0], phi_avh09, phi_slope, phi_offset, phi_fillvalue, ERROR=ERROR
  if keyword_set(ERROR) then return, -1
  ; phi "correction"
  ...
  sourceFileName=fsObj->getFileNameInfo(resFName, filePath=filePath, extension=extension)
  sourceFileName=fsObj->removeFileExtension(sourceFileName)
  outFName=fsObj->addFileExtension(outputDir+sourceFileName, 'sav')
