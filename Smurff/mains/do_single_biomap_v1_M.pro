FUNCTION do_single_biomap_v1_M, periodType, $
  ...
  biomapFQFile=mainApp->getKeyValue('BIOMAP_V1_FQ_FILENAME')
  ...
  if ((keyword_set(overwriteFlag) and count gt 0) or (count eq 0)) then begin
    ...
    bioMapFile=fs->removeFileExtension(roiSeaWiFSFile)
    
    bioMapOp=obj_new('BioMapOperatorV1', mainApp, tempDir, confFile=biomapConfFile, /ENVITYPE, fileName=bioMapFile, bandToExportList=bandToExportList)
    productCodes = bioMapOp->getBandToExportList()
    ;productCodes = bioMapOp->getParameterList()
    ;finalBioMapFile=bioMapOp->doComputation(roiArchiveCode, productCodes[0], sensorCode, applyfoq=applyfoq, ignoreValue=ignoreValue, textInputFile=textInputFile, pixelMask=pixelMask, NO_DATA=NO_DATA)
    
    for i=0, n_elements(productCodes)-1 do begin
      NO_DATA=0
      finalBioMapFile=bioMapOp->doComputation(roi, productCodes[i], sensorCode, applyfoq=applyfoq, ignoreValue=ignoreValue, textInputFile=textInputFile, pixelMask=pixelMask, NO_DATA=NO_DATA)
      if keyword_set(NO_DATA) then break
    endfor
    NOTFOUND=NO_DATA
    if ~keyword_set(NO_DATA) then begin
      bioMapOp->writeAsNCDF, month, year, sensorCode, roi, archiveDir=outputDir, overwriteFlag=overwriteFlag, fileToOverwriteList=fileToOverwriteList, applyFormula=applyFormula
