FUNCTION FileSystem::adjustDirSep, folder, ADD=ADD, REMOVE=REMOVE

 slashPos=strpos(folder, path_sep(), /REVERSE_SEARCH)
 newFolder=folder
 if slashPos eq strlen(folder)-1 then newFolder=strmid(newFolder, 0 , slashPos)
 if keyword_set(ADD) then newFolder=newFolder+path_sep()
 return, newFolder
