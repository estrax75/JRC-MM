FUNCTION readModelPSARois, buildFileNameFunction, periodType, getVarDataFunction, $
  ...
  for i=0, nroi-1 do begin
    ...
    doLog,'*start**readModelPSARois***', level=0
    ...
    ;buildFileNameFunction='buildModelPSAFileName'
    roiFileNames=call_function(buildFileNameFunction+'_'+periodType, $
      year, month, periodType, roiArchiveList[i], sensor, parameter, archiveRoot, FULLPATH=FULLPATH, JULDAY=JULDAY, INTERVAL=INTERVAL, TEMPDIR=tempDir, NOTFOUND=NOTFOUND1)
    ...
    for j=0, n_elements(roiFileNames)-1 do begin
      rawData = call_function(getVarDataFunction, roiFileNames[j], varInputList, varOutputList, convFuncList, tempDir, roiArchiveList[i], ignoreValue, NOTFOUND=NOTFOUND2);
      ;just skip this roi...
  ...
  if count ne 0 then begin
    ...
    mosaicFileName=tempDir+pathSep+'psa_euro'+'_'+strtrim(year)+'_'+strtrim(month)+'.envi'
    ...
    doLog,'*end**readModelPSARois***', level=0
    ...
    if nonGeoCount ne 0 then begin
      ...
      for i=0, nonGeoCount-1 do begin
        destFile=tempDir+pathSep+'psa_euro'+'_'+strtrim(year)+'_'+strtrim(month)+'_'+roiNames[i]+'.envi'
